...
// [END initializeEventLogger]
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        String bucket = "playchat-c89e7.appspot.com";
        int fileNum = 1;
        String filename = "gcs-store-file"; //name of the file. will be overwritten
        String fileExt = ".txt";

        GcsFileOptions instance = GcsFileOptions.getDefaultInstance();
        GcsFilename fileName = new GcsFilename(bucket, filename + fileExt);
        //check if the file exists
        while (gcsService.getMetadata(fileName) != null){
            fileName = new GcsFilename(bucket, filename + "(" + fileNum + ")" + fileExt);
            fileNum++;
        }

        GcsOutputChannel outputChannel;
        outputChannel = gcsService.createOrReplace(fileName, instance);
        PrintWriter writer = new PrintWriter(Channels.newWriter(outputChannel, "UTF8"), true);

        for (Iterator<LogEntry> iter = logs.iterator(); iter.hasNext(); ) {
            LogEntry entry = (LogEntry) iter.next();
            String displayText = new Date(entry.getTimeLong()).toString() + "(id=" + entry.getTag()
                    + ")" + " : " + entry.getLog();
            resp.getWriter().println(displayText);
